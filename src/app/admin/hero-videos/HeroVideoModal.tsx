...
export default function HeroVideoModal({ open, onClose, onSave, initialData, slotOrder }: HeroVideoModalProps) {
  ...
  async function handleThumbnailUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    setUploading(true);
    const { data, error } = await supabase.storage.from('thumbnails').upload(`hero-${slotOrder}-${file.name}`, file, { upsert: true });
    if (data) {
  ...
  async function handleVideoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    setUploading(true);
    const { data, error } = await supabase.storage.from('videos').upload(`hero-${slotOrder}-${file.name}`, file, { upsert: true });
    if (data) {
