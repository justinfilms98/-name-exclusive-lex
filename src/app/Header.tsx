...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-[#654C37] bg-opacity-90 py-1 m-0 shadow-md" style={{backdropFilter: 'blur(6px)'}}>
      <nav className="flex items-center w-full px-4 h-14 relative">
        ...
        <div className="flex items-center min-w-[48px]">
          ...
          >
            <svg className="w-6 h-6 text-[#D4C7B4]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
          </button>
          <Link href="/collections" className="text-[#D4C7B4] font-semibold hover:underline px-2 py-1 hidden sm:inline link-underline">Collections</Link>
        </div>
        {/* Center: Title */}
        <div className="flex-1 flex justify-center">
          <Link href="/" className="text-[#D4C7B4] font-bold text-lg sm:text-xl hover:underline px-2 py-1 link-underline whitespace-nowrap">
            Exclusive Lex
          </Link>
        </div>
