-- Exclusive Lex Database Consolidation & Fix Migration (SIMPLE VERSION)
-- This migration only adds missing columns and creates new tables
-- No collection_videos migration to avoid column errors
...
-- =====================================================
-- STEP 11: Handle collection_videos table (SAFE - NO MIGRATION)
-- =====================================================

-- Just check if collection_videos exists and log it
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'collection_videos') THEN
        RAISE NOTICE 'collection_videos table exists - you may want to handle this manually later';
        RAISE NOTICE 'For now, the platform will work with the collections table directly';
    ELSE
        RAISE NOTICE 'collection_videos table does not exist - no action needed';
    END IF;
END $$;
