-- Exclusive Lex Database Consolidation & Fix Migration (FIXED VERSION)
-- This migration works with your actual schema
...
-- =====================================================
-- STEP 11: Handle collection_videos table (SAFE APPROACH)
-- =====================================================

-- First, let's check what columns actually exist in collection_videos
DO $$ 
DECLARE
    col_exists boolean;
BEGIN
    -- Check if collection_videos table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'collection_videos') THEN
        RAISE NOTICE 'collection_videos table exists';
        ...
        IF col_exists THEN
            RAISE NOTICE 'collection_id column exists in collection_videos';
            
            -- Migrate data using collection_id
            WITH cv AS (
                SELECT 
                    collection_id, 
                    jsonb_agg(
            ...
            RAISE NOTICE 'Successfully migrated collection_videos data using collection_id';
        ...
        ELSE
            RAISE NOTICE 'collection_id column does not exist in collection_videos - skipping migration';
            RAISE NOTICE 'You may need to manually handle the collection_videos table';
        END IF;
    ELSE
